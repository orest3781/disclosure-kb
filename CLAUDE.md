...
- `sources.json` is the only file to edit for the catalog. Run `node scripts/build-index.mjs` and `node scripts/build-review.mjs` afterwards and commit the regenerated `INDEX.md`, `catalog/`, `REVIEW.md` and `kb/README.md` (its corpus-numbers block is generated) with it.
...
- Never mark a source Adopt in `scripts/lib/review.mjs` without a matching recommendation in `notes/` or `kb/stack.md`.
