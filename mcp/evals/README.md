...
Answers were checked directly against `sources.json` and `kb/stack.md` on 2026-09-23 (re-checked after the gap scan). Counts and star numbers are tied to that snapshot: if `sources.json` changes (new sources, refreshed stars), re-verify the answers before trusting a score.
